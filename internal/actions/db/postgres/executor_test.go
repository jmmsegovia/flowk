...
func TestDescribePlatformDatabase(t *testing.T) {
	...
	if got := describePlatformDatabase(" prod ", "main", "public"); got != "platform prod, database main schema public" {
		t.Fatalf("unexpected description: %q", got)
	}
	if got := describePlatformDatabase("", "main", "public"); got != "[No platform found] database main schema public" {
...
func TestExecuteUnsupportedOperation(t *testing.T) {
	...
	_, _, err := Execute(context.Background(), config.PostgresConfig{}, "not-real", nil, "", "", "", "", nil, "", nil, nil)
	if err == nil || !strings.Contains(err.Error(), "unsupported Postgres operation") {
...
func TestDatabasesForOperation(t *testing.T) {
	...
	cfg := config.PostgresConfig{Databases: []config.PostgresDatabase{{Name: "db1", Username: "u", Password: "p"}, {Name: "db2", Username: "u", Password: "p"}}}
