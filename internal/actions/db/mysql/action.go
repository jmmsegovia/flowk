...
func (c *taskConfig) Validate() error {
	if len(strings.TrimSpace(string(c.RawPlatform))) == 0 {
		return fmt.Errorf("mysql task: platform is required")
	}
	if strings.TrimSpace(c.Operation) == "" {
		return fmt.Errorf("mysql task: operation is required")
	}
	normalizedOperation := strings.ToUpper(strings.TrimSpace(c.Operation))
	if normalizedOperation == "SQL" {
		if strings.TrimSpace(c.Database) == "" {
			return fmt.Errorf("mysql task: database is required for SQL operations")
		}
		if strings.TrimSpace(c.Command) == "" {
			return fmt.Errorf("mysql task: command is required for SQL operations")
		}
	}
	if normalizedOperation == "LOAD_CSV" {
		if strings.TrimSpace(c.Database) == "" {
			return fmt.Errorf("mysql task: database is required for LOAD_CSV operations")
		}
		if strings.TrimSpace(c.Table) == "" {
			return fmt.Errorf("mysql task: table is required for LOAD_CSV operations")
		}
		if strings.TrimSpace(c.FilePath) == "" {
			return fmt.Errorf("mysql task: file_path is required for LOAD_CSV operations")
		}
	}
	return nil
}
...
func decodeTask(data json.RawMessage) (taskConfig, error) {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		return cfg, fmt.Errorf("decoding mysql task payload: %w", err)
	}
...
func (c taskConfig) MySQLConfig() (config.MySQLConfig, error) {
	cfg, err := config.ParseMySQLConfig(c.RawPlatform)
	if err != nil {
		return config.MySQLConfig{}, fmt.Errorf("mysql task: %w", err)
	}
	return cfg, nil
}
...
func (action) Execute(ctx context.Context, payload json.RawMessage, execCtx *registry.ExecutionContext) (registry.Result, error) {
	...
	platformCfg, err := cfg.MySQLConfig()
	if err != nil {
		return registry.Result{}, err
	}
